...
import { getFamilySendWordList, deleteFamilySendWord } from "@/api/family";
...
function handleDelete(row) {
  loading.value = true;
  deleteFamilySendWord(row).then(r => onSearch());
  setTimeout(() => {
    loading.value = false;
  }, 500);
}
...
async function onSearch() {
  ...
  const { data } = await getFamilySendWordList(form);
...
<template>
  <div class="main">
    ...
    <PureTableBar title="家庭寄语列表" @refresh="onSearch">
      <template #buttons>
        <el-button
          type="primary"
